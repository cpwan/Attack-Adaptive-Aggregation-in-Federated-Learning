...
def main(args):
    ...
    for i in range(args.num_clients):
        ...
        if i in attacker_list_labelflipping:
            ...
            client_i=Attacker_Omniscient(i,model,trainData[i],optimizer,device,args.attacker_list_omniscient_scale)            
        else:
...
if __name__=="__main__":
    ...
    class Arguments():
        def __init__(self):
            ...
            self.num_clients=10
            self.output_folder='experiements-Omniscient-Scale'
...
#     loader=[('non_overlap_label','./data/non_overlap_loader.pk')]
    ...
    attacks=['Omniscient','Omniscient2','Omniscient3','Omniscient4','Omniscient5']
    
    attacker_list_omniscient_scale=[10,1,0.1,0.01,0]
                                
    attacker_list_omniscient={'Omniscient':[0],
                                'Omniscient2':[1,2],
                                 'Omniscient3':[3,6,9],
                                 'Omniscient4':[1,3,5,7],
                                 'Omniscient5':[0,2,4,6,8],}
    for attack in attacks:
        for scale in attacker_list_omniscient_scale:
            for (loader_type,loader_path) in loader:
                for gar in GAR:              
                    args.GAR=gar
                    args.loader_type=loader_type
                    args.loader_path=loader_path
                    args.attacks=f'{attack}_scale_{scale}'
                    args.attacker_list_labelflipping=[]
                    args.attacker_list_omniscient=attacker_list_omniscient[attack]
                    args.attacker_list_omniscient_scale=scale
